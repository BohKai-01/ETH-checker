...
# --- CoinGecko Fetch for Ethereum ---
def fetch_eth_data_coingecko(days=200):
...
# --- Streamlit UI ---
st.set_page_config(page_title="ETH Buy/Sell Signal", layout="centered")
st.title("🧠 Ethereum Buy/Sell Signal App")

# Input for current ETH price from eToro
etoro_price = st.number_input("🔢 Enter current ETH price from eToro (USD):", value=2500.00)

try:
    df = fetch_eth_data_coingecko()
    df = calculate_indicators(df)
    latest = df.iloc[-1]

    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    st.markdown(f"**Date/Time:** {now}")
    st.markdown(f"**ETH Price (CoinGecko):** ${latest['Close']:.2f}")
